...
Uso típico:
  python consolidar_global_2005_2023.py \
      --indir "/Documentos/UDD/RetC/datos/filtrados_region/CONSOLIDADOS"
...
def main():
    ...
    ap.add_argument("--indir", required=True, help="Carpeta que contiene los dos archivos a consolidar")
    ap.add_argument("--efp-name", default="EFP_RM_2005_2022_consolidado.csv")
    ap.add_argument("--r23-name", default="ruea-efp-2023-ckan_RM.csv")
    ...
    efp_path = indir / args.efp-name if hasattr(args, 'efp-name') else indir / args.efp_name
    r23_path = indir / args.r23-name if hasattr(args, 'r23-name') else indir / args.r23_name

    # Fallback por nombres de atributo válidos en Python
    if not efp_path.exists():
        efp_path = indir / getattr(args, 'efp_name', 'EFP_RM_2005_2022_consolidado.csv')
    if not r23_path.exists():
        r23_path = indir / getattr(args, 'r23_name', 'ruea-efp-2023-ckan_RM.csv')
