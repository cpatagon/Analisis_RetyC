...
# -*- coding: utf-8 -*-
"""
Filtra TODOS los archivos descargados en ../datos/descargas_retc por región y
escribe los resultados en ../datos/filtrados_region.
...
Si existe el archivo de diagnóstico ../datos/diagnostico_archivos_originales/diagnostico_headers.csv,
...
def main():
    ...
    ap.add_argument("--root", default=None, help="Ruta a la raíz del proyecto que contiene la carpeta 'datos/' (opcional)")
    args = ap.parse_args()
    ...
    in_dir = root / "datos" / "descargas_retc"
    out_dir = root / "datos" / "filtrados_region"
    diag_dir = root / "datos" / "diagnostico_archivos_originales"
    ...
    if efp_consol:
        ...
        efp_all.to_csv(out_dir / "ruea_efp_RM_consolidado.csv", index=False, encoding="utf-8-sig")
        efp_all.to_excel(out_dir / "ruea_efp_RM_consolidado.xlsx", index=False)
    if r2023_consol:
        r23_all = pd.concat(r2023_consol, ignore_index=True)
        r23_all.to_csv(out_dir / "ckan_ruea_2023_RM_consolidado.csv", index=False, encoding="utf-8-sig")
        r23_all.to_excel(out_dir / "ckan_ruea_2023_RM_consolidado.xlsx", index=False)
